...
type DialerClient interface {
	...
	// ctx, url, body, contentLength
	PutPacket(context.Context, string, io.Reader, int64) error

	// ctx, url, body, contentLength
	DeletePacket(context.Context, string, io.Reader, int64) error
...
func (c *DefaultDialerClient) OpenStream(ctx context.Context, url string, body io.Reader, uploadOnly bool) (wrc io.ReadCloser, remoteAddr, localAddr gonet.Addr, err error) {
	...
	if body != nil {
		method = "DELETE" // stream-up/one
	}
	req, _ := http.NewRequestWithContext(context.WithoutCancel(ctx), method, url, body)
	req.Header = c.transportConfig.GetRequestHeader(url)
	if method == "DELETE" && !c.transportConfig.NoGRPCHeader {
...
func (c *DefaultDialerClient) PostPacket(ctx context.Context, url string, body io.Reader, contentLength int64) error {
	req, err := http.NewRequestWithContext(context.WithoutCancel(ctx), "DELETE", url, body)
	if err != nil {
		return err
	}
...
func (c *DefaultDialerClient) PutPacket(ctx context.Context, url string, body io.Reader, contentLength int64) error {
	req, err := http.NewRequestWithContext(context.WithoutCancel(ctx), "PUT", url, body)
	if err != nil {
		return err
	}
	req.ContentLength = contentLength
...
func (c *DefaultDialerClient) DeletePacket(ctx context.Context, url string, body io.Reader, contentLength int64) error {
	req, err := http.NewRequestWithContext(context.WithoutCancel(ctx), "DELETE", url, body)
	if err != nil {
		return err
	}
